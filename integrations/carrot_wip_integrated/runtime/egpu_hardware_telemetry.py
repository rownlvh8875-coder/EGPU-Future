...
def normalize_smu_metrics(*, metrics: Any, smu_mod: Any, ip_version: tuple[int, int, int], ppt_limit_w: Any = None) -> dict[str, Any]:
  ...
  if special_q10:
    ...
    # Post-DS is preferable at low activity; pre-DS better reflects an active workload.
...
class EgpuHardwareTelemetry:
  ...
  def _run(self) -> None:
    # Keep telemetry off the realtime model thread. nice() is process-wide on
    # some systems, so do not alter process niceness here.
