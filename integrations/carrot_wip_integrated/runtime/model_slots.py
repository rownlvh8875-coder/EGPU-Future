...
class ModelSlot:
  ...
  def validate(self) -> None:
    ...
    # Stage 3 metadata is intentionally not allowed to authorize model output
    # for controls. That decision remains in the existing Carrot modeld path.
...
class ModelSlotRegistry:
  ...
  def startup_preference(self, *, egpu_ready: bool) -> str:
    """Describe startup preference only; does not switch or load any model."""
    self.validate()
...
def egpu_slot_from_carrot_manifest(manifest: Any, *, ref: str | None = None, generation: int = 0,
  ...
  """Adapt Carrot BigModelManifest/dict without changing its download flow."""
  ...
  size = int(get("size"))
  sha256 = str(get("sha256")).lower()
  artifact = ModelArtifact(filename, size, sha256)
  slot = ModelSlot(
    slot=SLOT_EGPU,
    model_id=model_id,
    ref=ref or model_id,
...
def registry_to_dict(registry: ModelSlotRegistry) -> dict[str, Any]:
  ...
  return {
    ...
    "slots": {
      SLOT_QCOM: asdict(registry.qcom),
      SLOT_EGPU: asdict(registry.egpu) if registry.egpu is not None else None,
    },
    "policy": {
...
def _artifact_from_dict(value: Any) -> ModelArtifact | None:
  ...
  return ModelArtifact(
    file_name=str(value.get("file_name", value.get("fileName", ""))),
...
def _slot_from_dict(value: Any) -> ModelSlot:
  ...
  slot = ModelSlot(
    ...
    model_id=str(value.get("model_id", value.get("modelId", ""))),
    ref=str(value.get("ref", "")),
    backend=str(value.get("backend", "")),
    runner=str(value.get("runner", "")),
    generation=int(value.get("generation", 0)),
    nominal_hz=float(value.get("nominal_hz", value.get("nominalHz", 0.0))),
    source=str(value.get("source", "")),
    artifact=_artifact_from_dict(value.get("artifact")),
    builtin=bool(value.get("builtin", False)),
    control_eligible=bool(value.get("control_eligible", value.get("controlEligible", False))),
  )
  slot.validate()
  return slot
...
def registry_from_dict(value: Any) -> ModelSlotRegistry:
  ...
  qcom_raw = slots.get(SLOT_QCOM)
  egpu_raw = slots.get(SLOT_EGPU)
  registry = ModelSlotRegistry(
    qcom=_slot_from_dict(qcom_raw),
    egpu=_slot_from_dict(egpu_raw) if egpu_raw is not None else None,
    fallback_slot=str(value.get("fallbackSlot", SLOT_QCOM)),
...
def verify_artifact(path: str | Path, artifact: ModelArtifact, *, chunk_size: int = 4 * 1024 * 1024) -> bool:
  """Verify a local artifact without downloading, selecting, or loading it."""
  artifact.validate()
  file_path = Path(path)
  try:
